...
public class MappingTests
{
    ...
    {
        _OutputPath = Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location)!, "TestResultImages");
    ...
    public static void RegionMapping(bool equalArea = false, string? filePrefix = null, string? fileSuffix = null, bool pregen = false)
    {
        ...
        if (pregen)
        {
            using var img = Image.Load<L16>(Path.Combine(folder!, "elevation_5400x2700.png"));
        ...
        else
        {
            ...
            //if (ElevationMapResolution < ProjectionResolution) // uncomment as needed
            //{
        ...
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}Regional_ElevationImage{fileSuffix}.png"));
        }
        using (var img = elevationMap
            .ElevationMapToImage(planet, _HillShading))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}Regional_HillShadeImage{fileSuffix}.png"));
        }
        stopwatch1.Stop();
        ...
        //if (TemperatureMapResolution < ProjectionResolution) // uncomment as needed
        //{
        ...
        using (var img = temperatureMap.TemperatureMapToImage(
            ...
            false))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}Regional_LandTemperatureImage{fileSuffix}.png"));
        }
        ...
        using (var img = precipitationMap.PrecipitationMapToImage(
            ...
            false))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}Regional_LandPrecipitationImage{fileSuffix}.png"));
        }
        ...
        using (var img = weatherMaps.BiomeMap.BiomeMapToImage(
            ...
            _HillShading))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}Regional_BiomeOceanHillShadeImage{fileSuffix}.png"));
        }
        ...
        using var satelliteImg = weatherMaps.GetSatelliteImage(
            ...
            projection,
            projection);
        satelliteImg.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}Regional_SatelliteImage{fileSuffix}.png"));
        stopwatch2.Stop();
        ...
        bool pregen = false)
    {
        ...
        if (pregen)
        {
            elevationMap = Image.Load<L16>(Path.Combine(folder!, "elevation_5400x2700.png"));
        ...
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}ElevationImage{fileSuffix}.png"));
        }
        using (var img = elevationMapImage.ElevationMapToImage(planet, _HillShading))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}HillShadeImage{fileSuffix}.png"));
        }
        stopwatch1.Stop();
        ...
        using (var img = temperatureMap.TemperatureMapToImage())
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}TemperatureImage{fileSuffix}.png"));
        }
        using (var tImg = temperatureMap.TemperatureMapToImage(planet, elevationMap, false, projection, projection))
        {
            tImg.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}LandTemperatureImage{fileSuffix}.png"));
        }
        ...
        using (var snowfallMap = SurfaceMapImage.AverageImages(snowfallMaps))
        {
            using var snowfallImg = snowfallMap.SurfaceMapToImage(
                ...
                projection,
                projection);
            snowfallImg.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}SnowfallImage{fileSuffix}.png"));
        }
        ...
        using (var img = precipitationMap.PrecipitationMapToImage())
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}PrecipitationImage{fileSuffix}.png"));
        }
        using (var img = precipitationMap.PrecipitationMapToImage(planet, elevationMap, false, projection, projection))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}LandPrecipitationImage{fileSuffix}.png"));
        }
        ...
        var seasonsPath = Path.Combine(_OutputPath, "Seasons");
        ...
        for (var i = 0; i < Seasons; i++)
        {
            ...
            precipitationMaps[i]
                .SaveAsPng(Path.Combine(_OutputPath!, @$"Seasons\{filePrefix}PrecipitationImage{fileSuffix}_{i:00}.png"));
        }
        ...
        using (var img = weatherMaps.SeaIceRangeMap.SurfaceMapToImage(
            ...
            projection))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}SeaIceImage{fileSuffix}.png"));
        }
        ...
        using (var img = weatherMaps.ClimateMap.SurfaceMapToImage(
            ...
            _HillShading))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}ClimateImage{fileSuffix}.png"));
        }
        ...
        using (var img = weatherMaps.BiomeMap.BiomeMapToImage(
            ...
            _HillShading))
        {
            img.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}BiomeOceanHillShadeImage{fileSuffix}.png"));
        }
        ...
        using var satelliteImg = weatherMaps.GetSatelliteImage(
            ...
            projection,
            projection);
        satelliteImg.SaveAsPng(Path.Combine(_OutputPath!, $"{filePrefix}SatelliteImage{fileSuffix}.png"));
        stopwatch2.Stop();
