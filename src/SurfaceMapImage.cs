            ...
            MapProjectionOptions? elevationMapProjection = null)
        {
            ...
            return maps.BiomeMap.SurfaceMapToImageByLatLon(
                  ...
                  (v, e, lat, lon) =>
                  {
                      ...
                      return new Rgba32(
                          (byte)Math.Round(MathExtensions.Lerp(pixel.R, 255, elevationFactor)),
                          (byte)Math.Round(MathExtensions.Lerp(pixel.G, 255, elevationFactor)),
                          (byte)Math.Round(MathExtensions.Lerp(pixel.B, 255, elevationFactor)));
                  },
                  (_, e, x, y) =>
                  {
